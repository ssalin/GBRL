...
static const u1_t PROGMEM DEVEUI[8]={ 0xB1, 0xB4, 0xCF, 0xB5, 0x8A, 0x31, 0x75, 0x00 };
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

// This key should be in big endian format
static const u1_t PROGMEM APPKEY[16] = { 0x2B, 0x52, 0xFB, 0x3C, 0x5E, 0xA5, 0x16, 0x7C, 0xC0, 0x8D, 0x66, 0x8D, 0xC4, 0x0C, 0x27, 0x33 };
...
int logdata() {
  ...
  if (W_File) {
    ...
    if (firstrun) {
      ...
      W_File.println(F("Date,Time,Temperature(Celcius), RTC_TEMP (Celcius) ,Humidity(%),Particles(pcs/.01 cubic feet),Ozone(RAW,PPM),CO(RAW,PPM)"));
      W_File.println();
    }
...
void senddata(){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        Serial.println(F("Packet queued and logged"));
    }
    // Next TX is scheduled after TX_COMPLETE event.
}
